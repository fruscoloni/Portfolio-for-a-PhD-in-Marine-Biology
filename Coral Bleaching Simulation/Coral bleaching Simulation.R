...
# Mixed model
pred_species <- ggpredict(model, terms = c("Temperature_C [all]", "Species"))
lmm_plot <- ggplot() +
  geom_point(
    data = coral_final, 
    aes(x = Temperature_C, y = `Bleaching_%`, color = Species, shape = Depth_m), 
    size = 2.5, alpha = 0.5
  ) +
  geom_ribbon(
    data = pred_species, 
  ...
  labs(
    tag = "C",
    title = "LMM Predicted Bleaching Trajectories by Species",
    subtitle = "Lines and 95% CIs represent fixed-effect predictions from the mixed model",
    x = "Temperature (°C)",
    y = "Bleaching (%)",
    shape = "Depth",
    color = "Species"
...
#
bleaching_plots <- A | B
ggsave("Bleaching_plots.png", bleaching_plots, path = "Plots", width = 15, height = 7, dpi = 300)
ggsave("Mixed_model.png", lmm_plot, path = "Plots", width = 10, height = 7, dpi = 300)
